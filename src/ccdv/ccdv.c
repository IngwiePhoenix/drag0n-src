...
#define SETCOLOR_SUCCESS	(gANSIEscapes ? "\033[01;32m" : "")
#define SETCOLOR_FAILURE	(gANSIEscapes ? "\033[01;31m" : "")
#define SETCOLOR_WARNING	(gANSIEscapes ? "\033[01;33m" : "")
#define SETCOLOR_NORMAL		(gANSIEscapes ? "\033[00;39m" : "")
...
#define TERMS "vt100:vt102:vt220:vt320:xterm:xterm-color:xterm-256color:ansi:linux:scoterm:scoansi:dtterm:cons25:cygwin"
...
main(int argc, char **argv)
{
	...
	for (i = 1; i < argc; i++) {
		...
		if((strcmp(argv[i], "-c") == 0) && ((i + 1) < argc)) {
			...
			haveSource=TRUE;
		} else if ((strcmp(argv[i], "-o") == 0) && ((i + 1) < argc) && haveSource==FALSE) {
			if (strcasecmp(Extension(argv[i + 1]), ".o") != 0 || strcasecmp(Extension(argv[i + 1]), ".lo") != 0) {
				strcpy(gAction, "Linking");
				snprintf(gTarget, sizeof(gTarget), "%s", Basename(argv[i + 1]));
			}
		} else if (strchr("-+/", (int) argv[i][0]) != NULL) {
			continue;
		} else if (strncasecmp(Extension(argv[i]), ".c", 2) == 0 || strncasecmp(Extension(argv[i]), ".cpp", 4) == 0) {
			cc++;
